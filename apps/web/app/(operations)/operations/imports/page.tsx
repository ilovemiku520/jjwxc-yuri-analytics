...
export default async function ImportsPage() {
  ...
  try {
    report = await loadJjwxcProbeView();
  } catch {}
    ...
    <main className="catalog-page">
      ...
      <section className="import-decision" aria-labelledby="jjwxc-probe-title">
        <div>
          ...
          <p>
            公开页与普通读者登录页都不提供 V 章节点击；V
            点击只接受作品作者本人从作者后台导出的授权数据，缺失时保持为空。
          </p>
        </div>
        <dl>
          <div>
            <dt>外部请求</dt>
            <dd className={ready ? "safe-value" : undefined}>
              {report?.requestCount ?? 0} / 1
            </dd>
          </div>
          <div>
            <dt>候选记录</dt>
      ...
      <section className="state-panel">
        ...
        <p>
          双击 <code>scripts/run-jjwxc-public-probe.cmd</code>{" "}
          可用默认样本；也可从终端传入一个公开 novelid。普通读者 Cookie
          不会被用于推断或伪造 V 点击。
        </p>
      </section>
      <section className="state-panel">
      ...
      <section className="operations-grid" aria-label="JJWXC 采集边界">
        <article className="state-panel">
          ...
          <p>
            仅允许 <code>https://www.jjwxc.net/onebook.php?novelid=…</code>
            ，拒绝跳转与其他主机。
          </p>
        </article>
        <article className="state-panel">
          ...
          <p>
            当前阶段单并发、单请求、零自动重试。后续批次也必须采用低频快照而非秒级实时抓取。
          </p>
        </article>
        <article className="state-panel">
          <div className="panel-heading">
          ...
          <p>
            文案、章节标题、正文、评论正文、读者身份、付费内容与原始响应全部丢弃。
          </p>
        </article>
        <article className="state-panel">
          <div className="panel-heading">
          ...
          <p>仅供个人学习或研究，严禁商业用途、二次分发或数据镜像。</p>
        </article>
      </section>
    </main>
  );
