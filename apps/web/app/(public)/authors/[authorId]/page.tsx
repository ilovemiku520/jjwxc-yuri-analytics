...
export default async function AuthorDetailPage({ params }: { params: Promise<{ authorId: string }> }) {
  ...
  return (
    <main className="catalog-page">
      ...
      <header className="page-heading detail-heading"><div><p className="eyebrow">JJWXC authorid {author.author_id}</p><h1>{author.author_display_name}</h1></div><p>{formatCount(author.novel_count)} 部已统计百合小说 · {author.profile_locked_work_count ?? "—"} 部锁定作品已排除</p></header>
      <section className="stat-grid author-stat-grid"><Stat label="非锁定作品" value={author.profile_nonlocked_work_count} source="作者专栏" /><Stat label="作者收藏" value={author.profile_author_favorite_count} source="作者专栏" /><Stat label="作品总收藏" value={author.total_favorite_count} source="已统计作品" /><Stat label="写文总字数" value={author.profile_total_word_count} source="非锁定作品" /><Stat label="总积分" value={author.profile_total_points} source="非锁定作品" /></section>
...
function Stat({ label, value, source }: { label: string; value: number | null; source: string }) { return <article className="stat-card"><p className="stat-label">{label}</p><p className="stat-value">{value === null ? "待采集" : formatCount(value)}</p><p className="stat-note">{source} · 公开聚合快照</p></article>; }
