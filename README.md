...
运维状态，不在长期数据库中保存小说正文、评论内容或文案原文。

项目所属与作者：ilovemiku520@outlook.com

## 当前能力

- 可扩展作品索引：按作品名或作者名查询 PostgreSQL 中文子串索引，再按标签、状态和指标细分；
...
- 可恢复频道语料回填：频道页面本次发现 412 个去重作品编号，进入数据库队列后分批补全并缓存 24 小时；
- 单样本真实探针：一次只访问一个公开作品概览页，无登录、无 Cookie、无自动重试；
- 既有工程底座：PostgreSQL、Alembic、FastAPI、Next.js、Docker、安全门禁与审计模型。
- PostgreSQL 快照底座：迁移 `20260824_0012` 提供作者、小说、章节点击、双榜单、发现队列与搜索索引。
...
`synopsis_theme_terms`、`public_tags`、双榜单位置、`observed_at`。
